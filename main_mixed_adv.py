    ...
    while True:
        try:
            ...
            if config.ADD_ADVERSARIAL_ATTACK:
                ...
                delta.data = config.ATTACK_ALPHA * grad  # delta是个torch变量，数值赋值是赋给data的
                delta = delta.detach()
...
def split_samestate_for_train_eval_test(x_data_all_x, event_action_all_x, y_trials_all_x, y_label_all_x):
    """
    train:eval:test = 8:1:1 split in trials
    ...
    train_eval_data, test_data, unused, event_action_test, unused, test_trials, train_eval_label, unused = \
        train_test_split(x_data_all_x, event_action_all_x, y_trials_all_x, y_label_all_x,
                         random_state=RANDOM_SEED, train_size=9.0 / 10,
                         test_size=1.0 / 10, shuffle=True)
    # for train and valid
    train_data, eval_data, train_label, eval_label = train_test_split(train_eval_data, train_eval_label,
                                                                      random_state=RANDOM_SEED, train_size=8.0 / 9,
...
def split_differstate_for_train(x_data_all_x, y_label_all_x):
    """
        train:eval_test = 8:2
        input：x_data[0]，y_label[0] are reps
        return:train dataset
        _x: possible state
        """
    train_data, unused, train_label, unused = train_test_split(
        x_data_all_x, y_label_all_x, random_state=RANDOM_SEED, train_size=8.0 / 10,
        test_size=2.0 / 10, shuffle=True)
    return train_data, train_label


def split_differstate_for_eval_test(x_data_all_x, y_label_all_x, event_action_all_x, y_trials_all_x):
    """
        eval:test = 8:1:1
    ...
    event_action_all_x = event_action_all_x.reshape(-1, 10, 12)
    y_label_all_x = y_label_all_x.reshape(-1, 10, 1)
    # split in the dimension of trials
    unused, eval_test_data, unused, eval_label, unused, event_action_test, unused, test_trials = train_test_split(
        x_data_all_x, y_label_all_x, event_action_all_x, y_trials_all_x, random_state=RANDOM_SEED, train_size=8.0 / 10,
        test_size=2.0 / 10, shuffle=True)
    eval_data, test_data, eval_label, unused, unused, event_action_test, unused, test_trials = train_test_split(
        eval_test_data, eval_label, event_action_test, test_trials, random_state=RANDOM_SEED, train_size=1.0 / 2,
...
def train_early_stopping(model, x_data_all_L, x_data_all_M, x_data_all_H, y_label_all_L, y_label_all_M, y_label_all_H,
                         ...
                         y_trials_all_H, state_train, state_test, optimizer,
    ...
    if not state_train == state_test:
        if state_train == 0:
            train_data, train_label = split_differstate_for_train(x_data_all_L, y_label_all_L)
        if state_train == 1:
            train_data, train_label = split_differstate_for_train(x_data_all_M, y_label_all_M)
        if state_train == 2:
            train_data, train_label = split_differstate_for_train(x_data_all_H, y_label_all_H)

        if state_test == 0:
            eval_data, eval_label, test_data, test_trials, event_action_test = split_differstate_for_eval_test \
                (x_data_all_L, y_label_all_L, event_action_all_L, y_trials_all_L)
        if state_test == 1:
            eval_data, eval_label, test_data, test_trials, event_action_test = split_differstate_for_eval_test \
                (x_data_all_M, y_label_all_M, event_action_all_M, y_trials_all_M)
        if state_test == 2:
            eval_data, eval_label, test_data, test_trials, event_action_test = split_differstate_for_eval_test \
                (x_data_all_H, y_label_all_H, event_action_all_H, y_trials_all_H)

    if state_train == state_test:  # state_train == state_test: L-L, M-M, H-H
        if state_train == 0:  # split randomly
            train_data, train_label, eval_data, eval_label, test_data, event_action_test, test_trials = \
                split_samestate_for_train_eval_test(x_data_all_L, event_action_all_L, y_trials_all_L, y_label_all_L)
        if state_train == 1:
            train_data, train_label, eval_data, eval_label, test_data, event_action_test, test_trials = \
                split_samestate_for_train_eval_test(x_data_all_M, event_action_all_M, y_trials_all_M, y_label_all_M)
        if state_train == 2:
            train_data, train_label, eval_data, eval_label, test_data, event_action_test, test_trials = \
                split_samestate_for_train_eval_test(x_data_all_H, event_action_all_H, y_trials_all_H, y_label_all_H)
...
def main():
    ...
    if config.IS_TRAIN:
        ...
        if not config.LOAD_DATA_PICKLE:
            x_data_all_L, x_data_all_M, x_data_all_H, y_label_all_L, y_label_all_M, y_label_all_H, event_action_all_L, \
            event_action_all_M, event_action_all_H, y_trials_all_L, y_trials_all_M, y_trials_all_H = \
                get_data_from_files(config.DATA_FOLDER)
            print('')
            ...
            x_data_all_L, x_data_all_M, x_data_all_H, y_label_all_L, y_label_all_M, y_label_all_H, event_action_all_L \
                , event_action_all_M, event_action_all_H, y_trials_all_L, y_trials_all_M, y_trials_all_H = pickle.load(
                data_pkl_f)
            data_pkl_f.close()

        # 9states 1-10reps 5个seeds
        for state_train in range(0, 3):
            for state_test in range(0, 3):
                ...
                model = ActionPredBasedEEG(win_for_eeg_sig=config.WIN_FOR_EEG_SIGNAL,
                                           ...
                                           eeg_fea_dim=config.EEG_FEATURE_DIM, rnn_hid_dim=config.RNN_HIDDEN_DIM,
                ...
                if config.RELOAD_PRETRAIN_MODEL and config.MODEL_SAVE_PATH is not None:
                    params_path = '%s%05d%s' % (params_path_toks[0], config.RELOAD_EVAL_EPOCH, params_path_toks[1])
                ...
                # TIMESTAMP = format_time(time.localtime())
                EVENTSTAMP = "mixed_%d-%d" % (state_train, state_test)
                ...
                train_early_stopping(model, x_data_all_L, x_data_all_M, x_data_all_H,
                                     ...
                                     optimizer, criterion_action, params_path_toks, logger, lr_decay=lr_decay,
                                     early_stop=early_stop)
    else:
