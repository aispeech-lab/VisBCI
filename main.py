...
from models import ActionPredBasedEEG
...
def split_differstate_for_eval_test(x_data_all_x, y_label_all_x, event_action_all_x, y_trials_all_x, test_index):
    """
        object: select one person as target. when train state unequals test state, eval:test=1:1；
        input：x_data[0]，y_label[0], event_action_all[0] are reps，y_trials[0] are trials；
        return：valid dataset(the first dimension of all are reps) & test dataset (the first dimension of all are trials)
         _x: three possible states
        """
    # y_trials_all_x[0] are trials already
    x_data_all_x = x_data_all_x.reshape(-1, 10, 12, 28, 600)  # [0] trials
    event_action_all_x = event_action_all_x.reshape(-1, 10, 12)
    y_label_all_x = y_label_all_x.reshape(-1, 10, 1)
    eval_test_data = x_data_all_x[test_index]
    y_label_all_x = y_label_all_x[test_index]
    event_action_all_x = event_action_all_x[test_index]
    y_trials_all_x = y_trials_all_x[test_index]
    eval_data, test_data, eval_label, unused, unused, event_action_test, unused, test_trials = train_test_split(
        eval_test_data, y_label_all_x, event_action_all_x, y_trials_all_x, random_state=RANDOM_SEED, train_size=1.0 / 2,
        test_size=1.0 / 2, shuffle=True)
    # for valid dataset
    eval_data = eval_data.reshape(-1, 12, 28, 600)
    eval_label = eval_label.reshape(-1, 1)
    return eval_data, eval_label, test_data, test_trials, event_action_test


def split_samestate_for_train_eval_test(x_data_all_x, event_action_all_x, y_trials_all_x, y_label_all_x, test_index):
    """
    object: select one person as target. when train state equals test state, train:eval:test=4:1:1 split in trials；
    input：x_data[0]，y_label[0], event_action_all[0] are reps，y_trials[0] are trials；；
    return：train, valid dataset(1d reps) & test dataset (1d trials)
    _x: three possible states
    """
    x_data_all_x = x_data_all_x.reshape(-1, 10, 12, 28, 600)
    x_data_all_x = x_data_all_x[test_index]
    y_label_all_x = y_label_all_x.reshape(-1, 10, 1)
    y_label_all_x = y_label_all_x[test_index]
    event_action_all_x = event_action_all_x.reshape(-1, 10, 12)
    event_action_all_x = event_action_all_x[test_index]
    y_trials_all_x = y_trials_all_x[test_index]
    # for test
    train_eval_data, test_data, unused, event_action_test, unused, test_trials, train_eval_label, unused = \
        train_test_split(x_data_all_x, event_action_all_x, y_trials_all_x, y_label_all_x,
                         random_state=RANDOM_SEED, train_size=5.0 / 6,
                         test_size=1.0 / 6, shuffle=True)
    # for train and valid
    train_data, eval_data, train_label, eval_label = train_test_split(train_eval_data, train_eval_label,
                                                                      random_state=RANDOM_SEED, train_size=4.0 / 5,
                                                                      test_size=1.0 / 5, shuffle=True)
    train_data = train_data.reshape(-1, 12, 28, 600)  # [0]reps
    train_label = train_label.reshape(-1, 1)
    eval_data = eval_data.reshape(-1, 12, 28, 600)  # [0]reps
    eval_label = eval_label.reshape(-1, 1)
...
def train_early_stopping(model, x_data_all_L, x_data_all_M, x_data_all_H, y_label_all_L, y_label_all_M, y_label_all_H,
    ...
    if not state_train == state_test:
        if state_train == 0:
            x_data_all_L = x_data_all_L.reshape(-1, 10, 12, 28, 600)  # reshape [0] trials;
            y_label_all_L = y_label_all_L.reshape(-1, 10, 1)
            x_data_all_L = x_data_all_L[test_index]
            y_label_all_L = y_label_all_L[test_index]
            train_data = x_data_all_L.reshape(-1, 12, 28, 600)
            train_label = y_label_all_L.reshape(-1, 1)
        if state_train == 1:
            x_data_all_M = x_data_all_M.reshape(-1, 10, 12, 28, 600)
            y_label_all_M = y_label_all_M.reshape(-1, 10, 1)
            x_data_all_M = x_data_all_M[test_index]
            y_label_all_M = y_label_all_M[test_index]
            train_data = x_data_all_M.reshape(-1, 12, 28, 600)
            train_label = y_label_all_M.reshape(-1, 1)
        if state_train == 2:
            x_data_all_H = x_data_all_H.reshape(-1, 10, 12, 28, 600)
            y_label_all_H = y_label_all_H.reshape(-1, 10, 1)
            x_data_all_H = x_data_all_H[test_index]
            y_label_all_H = y_label_all_H[test_index]
            train_data = x_data_all_H.reshape(-1, 12, 28, 600)
            train_label = y_label_all_H.reshape(-1, 1)

        # for valid and test divide in trials, return reps(1d) for valid, but trials(1d) for test
        if state_test == 0:
            eval_data, eval_label, test_data, test_trials, event_action_test = split_differstate_for_eval_test \
                (x_data_all_L, y_label_all_L, event_action_all_L, y_trials_all_L, test_index)
        if state_test == 1:
            eval_data, eval_label, test_data, test_trials, event_action_test = split_differstate_for_eval_test \
                (x_data_all_M, y_label_all_M, event_action_all_M, y_trials_all_M, test_index)
        if state_test == 2:
            eval_data, eval_label, test_data, test_trials, event_action_test = split_differstate_for_eval_test \
                (x_data_all_H, y_label_all_H, event_action_all_H, y_trials_all_H, test_index)

    if state_train == state_test:  # state_train == state_test: L-L, M-M, H-H
        if state_train == 0:
            train_data, train_label, eval_data, eval_label, test_data, event_action_test, test_trials = \
                split_samestate_for_train_eval_test(
                    x_data_all_L, event_action_all_L, y_trials_all_L, y_label_all_L, test_index)
        if state_train == 1:
            train_data, train_label, eval_data, eval_label, test_data, event_action_test, test_trials = \
                split_samestate_for_train_eval_test(
                    x_data_all_M, event_action_all_M, y_trials_all_M, y_label_all_M, test_index)
        if state_train == 2:
            train_data, train_label, eval_data, eval_label, test_data, event_action_test, test_trials = \
                split_samestate_for_train_eval_test(
                    x_data_all_H, event_action_all_H, y_trials_all_H, y_label_all_H, test_index)
    ...
    while True:
        ...
        if epoch_idx == config.MAX_EPOCH_NUM:
            print('[Stop Iter]-Best Epoch %d, total time cost (%s): best valid loss: %f and best valid acc: %f '
                  % (best_val_epoch, timeSince(start_time), early_stop.get_best_loss(), best_acc_valid))
            # save model's params
            if epoch_idx >= 1 and epoch_idx % config.SAVE_EPOCH == 0:
                print("[Saving Trained Parameters] at epoch %d and best epoch is %d, best valid acc is %f" % (
                    epoch_idx, best_val_epoch, best_acc_valid))
                model_dir = '%s%d-%d-%d%s' % (params_path_toks[0], state_train, state_test, RANDOM_SEED,
                                              params_path_toks[1])
                if config.USE_CUDA and len(config.GPUS) > 1:
                    torch.save(
                        {'state_dict': model.module.state_dict(),
    ...
    if config.IS_TRAIN:
        ...
        for state_train in range(0, 3):  #
            for state_test in range(0, 3):  #
                ...
                # divide the 10 subjects, 9 train, 1 test
                ...
                for train_index, test_index in kf.split(y_trials_all_L):
                    ...
                    if count in range(1, 11):
                        ...
                        EVENTSTAMP = "single_%d-%d-%d" % (state_train, state_test, count)
                        logdir = config.TENSORBOARD_LOG_DIR + EVENTSTAMP
                        if not os.path.exists(logdir):
                            os.mkdir(logdir)
                        logger = Logger(logdir)
                        train_early_stopping(model, x_data_all_L, x_data_all_M, x_data_all_H, y_label_all_L,
...
if __name__ == "__main__":
    ...
    for RANDOM_SEED in range(0, 1):
        ...
        print('Start, and the random seed is %d' % RANDOM_SEED)
        if config.USE_CUDA:
            # ensure the certainty and repeatability of the results
