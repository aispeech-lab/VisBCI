...
"""
build the 1dCNN+BiLSTM model with auxiliary normalization module
"""
...
class Encoder(nn.Module):
    ...
    def forward(self, in_samples):
        """
        Args:
            samples: [B, C, T], B is batch size, C is channels, T is #samples
        Returns:
...
class ActionPredBasedEEG(nn.Module):
    def __init__(self, win_for_eeg_sig, eeg_enc_dim, eeg_fea_dim, rnn_hid_dim, rnn_layer_num, dropout=0.5):
        ...
        self.win_for_eeg_sig = win_for_eeg_sig
        self.eeg_enc_dim = eeg_enc_dim
        self.eeg_fea_dim = eeg_fea_dim
        ...
        # separate clean and adv;  set an auxiliary GroupNorm
        self.erp_enc_LayerNorm = nn.GroupNorm(1, self.eeg_enc_dim, eps=1e-8)  # [B E L]-->[B E L]
        self.erp_enc_LayerNorm_adv = nn.GroupNorm(1, self.eeg_enc_dim, eps=1e-8)  # [B E L]-->[B E L]
        # bottleneck
        ...
        self.erp_global_rnn = nn.LSTM(self.rnn_hid_dim * (int(self.bidirectional) + 1), self.rnn_hid_dim,
                                      self.num_layers
    ...
    def forward(self, eeg_erp_in, ADD_ADVERSARIAL_ATTACK):
        """
        :param eeg_erp_in: shape (batch, erp_clips:12, channels:28, seq_len:600 ms / erp)
        """
        ...
        # add an auxiliary GroupNorm
        if ADD_ADVERSARIAL_ATTACK:
            eeg_erp_fea = self.erp_enc_LayerNorm_adv(eeg_erp_fea)
        else:
        ...
        # (batch*erp_clips, seq_len, fea_dim) -> (batch*erp_clips, fea_dim)
        eeg_erp_local_fea = torch.mean(eeg_erp_local_fea, dim=1).squeeze(0)  # mean pooling
